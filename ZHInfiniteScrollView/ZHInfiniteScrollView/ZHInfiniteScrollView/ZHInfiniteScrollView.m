...
@interface ZHInfiniteScrollView () <UIScrollViewDelegate>
...
@implementation ZHInfiniteScrollView
#pragma mark - 构造方法
#pragma mark -
#pragma mark Init方法
- (instancetype) initWithFrame:(CGRect)frame delegate:(id<ZHInfiniteScrollViewDelegate>)delegate dataSource:(id<ZHInfiniteScrollViewDataSource>)dataSource timerInterval:(NSTimeInterval)timeInterval{
...
+ (instancetype) infiniteScrollViewWithFrame:(CGRect)frame delegate:(id<ZHInfiniteScrollViewDelegate>)delegate dataSource:(id<ZHInfiniteScrollViewDataSource>)dataSource timeInterval:(NSTimeInterval)timeInterval {
    ...
    ZHInfiniteScrollView *scrollView = [[ZHInfiniteScrollView alloc] initWithFrame: frame delegate:delegate dataSource:dataSource timerInterval:timeInterval];
    return scrollView;
}
...
+ (instancetype) infiniteScrollViewWithFrame:(CGRect)frame delegate:(id<ZHInfiniteScrollViewDelegate>)delegate dataSource:(id<ZHInfiniteScrollViewDataSource>)dataSource {
    ...
    ZHInfiniteScrollView *scrollView = [ZHInfiniteScrollView infiniteScrollViewWithFrame: frame delegate:delegate dataSource:dataSource timeInterval: kDefaultTimeInterval];
    return scrollView;
}
...
+ (instancetype) infiniteScrollViewWithFrame:(CGRect)frame delegate:(id<ZHInfiniteScrollViewDelegate>)delegate dataSource:(id<ZHInfiniteScrollViewDataSource>)dataSource timeInterval:(NSTimeInterval)timeInterval inView:(UIView *)destinationView {
    ...
    ZHInfiniteScrollView *scrollView = [ZHInfiniteScrollView infiniteScrollViewWithFrame:frame delegate:delegate dataSource:dataSource timeInterval:timeInterval];
...
- (id) dequeueReusableCellWithIdentifier:(NSString *)cellIdentifier {
    ...
    // 5. 任意取出一个 可复用的 Cell
    ZHInfiniteScrollViewCell *cell = [reusableCellSet anyObject];
...
- (void) reloadData {
    ...
    for (NSInteger index=0; index<self.dataCount + 2; index++) {
        ...
        ZHInfiniteScrollViewCell *cell = [self.dataSource infiniteScrollView: self viewAtIndex: exactIndex];
        cell.index = exactIndex;
        cell.frame = CGRectMake(index*self.width, 0, self.width, self.height);
        [self.scrollView addSubview: cell];
        
        // 2.4 监听 Cell 的点击事件, 然后回调代理
        ZHInfiniteScrollView *__weak weakSelf = self;
    ...
    [self.reusableCellDictionary enumerateKeysAndObjectsUsingBlock:^(id key, NSSet *obj, BOOL *stop) {
        for (ZHInfiniteScrollViewCell *cell in obj) {
...
- (void) addReusableCell:(ZHInfiniteScrollViewCell *)cell inReusableDictionay:(NSMutableDictionary *)reusableDictionary {
